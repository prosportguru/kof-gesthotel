...
export default function Destinations({back,action}) {
    ...
    const del=async (item,set_deleting)=>{
        ...
        set_deleting(true)
        await db.collection("destinations").doc(item?.key).delete()
        await load_data()
        set_deleting(false)
    }
            ...
            <table width={"100%"} className='border'>
                ...
                <tbody className=''>
                    {data_show?.map((item,index)=>{
                        return(
                            <Destination item={item} index={index} key={index}  del={del} set_open_update={()=>{
                                set_selected(item)
