...
export default function Equipements({back,action}) {
    ...
    const load_data=async ()=>{
        const snap=await db.collection("equipements").get();
        let d=[];
        snap.docs.map((doc)=>{
            let id=doc.id;
            let dt=doc.data()
            ...
            <table width={"100%"} className='border'>
                ...
                <tbody className=''>
                    {data_show?.map((item,index)=>{
                        return(
                            <Equipement item={item} index={index} key={index}  del={del} set_open_update={()=>{
                                set_selected(item)
