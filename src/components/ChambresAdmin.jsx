...
import ChambreAdmin from "./ChambreAdmin";
...
export default function ChambresAdmin({back,action}) {
    ...
    const load_data=async ()=>{
        const snap=await db.collection("chambres").get();
        let d=[];
        snap.docs.map((doc)=>{
            let id=doc.id;
            let dt=doc.data()
    ...
    const del=async (item,set_deleting)=>{
        ...
        set_deleting(true)
        await db.collection("chambres").doc(item?.key).delete()
        await load_data()
        set_deleting(false)
    }
